...
the repo root. Store tests use a real ephemeral on-disk SQLite, not a mock. The optional web
package lives in `web/`; run `cd web && bunx vite build` to type-check and build the library.
CI runs the web build first, then `go vet ./...` and `go test -race ./...`. The Go core must
